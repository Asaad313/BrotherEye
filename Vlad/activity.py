from Vlad.local_libs.activity_search import ActivitySearch
from Vlad.commander import Commander
...
class Activity(Commander):
    ...
    def calculate(self, text):
        default_text_array = ['Vlad', 'wake', 'up']
        ...
        if command_text_array == default_text_array:
            chances2 = self.get_chances(['Vlad', 'wakeup'], user_text_array)
            if chances2 > chances:
                chances = chances2
        if chances > 70.0:
            return True
    ...
    def get_chances(self, given_keywords, user_keywords, exact=True):
        chances = self.ass.get_probability(given_keywords=given_keywords, user_keywords=user_keywords)
        if exact:
            return chances[0]
        return chances
