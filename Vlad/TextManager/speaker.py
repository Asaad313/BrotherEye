...
class Speaker:
    ...
    def speak_from_os(self, speech_result_filename):
        ...
        try:
            ...
            print("In Textmanager speaker Error: Set the defualt mp3 player like vlc or something.")
        try:
            self.hibernate()
        except:
            print("Eyed3 named package wasn't installed probably Check back at the support tab in the main website. Or if you're trying to close the application abruptly, keep pressing CTRL + C repeatedly.")

    @staticmethod
    def get_abs_filename(filename):
        speak_result = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir, filename))
        return speak_result
