...
class AudioGetter:
        ...
        try:
            if signals:
                if self.tts_option == "os":
                    ...
                    raise AssertionError("Audio Getter TTS_player error: Fill in the tts_player option in config.ini file as either os or mixer.")
            ...
            if signals:
                if self.tts_option == "os":
                    ...
                    raise AssertionError("In audio_getter TTS_player error: Fill in the tts_player option in config.ini file as either os or mixer.")
            return audio
    ...
    def listen(self, source, signals=True):
        try:
            ...
            raise Exception("In audio_getter Input error: Make sure some kind of speaking device is properly installed in your system.")

    def get_speeches_folder(self, filename):
        return os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir, self.speech_directory, filename))
